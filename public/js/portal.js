...
function initAuthForms() {
  ...
  dom.formPortalLogin?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      portalState.currentUser = data.user;
      localStorage.setItem('veltha_user', JSON.stringify(data.user));
      renderPortalState();
      showToast(`Bem-vindo, ${data.user.name}! Acesso liberado aos arquivos técnicos.`, 'success');
    } catch (err) {
  ...
  dom.formPortalRegister?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      localStorage.setItem('veltha_user', JSON.stringify(data.user));
      renderPortalState();
      showToast(`Cadastro realizado com sucesso! Bem-vindo, ${data.user.name}.`, 'success');
    } catch (err) {
